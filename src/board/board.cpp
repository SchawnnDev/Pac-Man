...
Board::Board() {
    ...
    for (int i = 0; i < BOARD_SIZE_Y; ++i) {
        for (int j = 0; j < BOARD_SIZE_X; ++j) {
            m_grid[i][j] = std::make_shared<BoardCase>(j,i, BoardCaseType::BasicPath);
        }
    }
...
Board::Board(const std::string& p_filePath) : Board() {
    ...
    std::cout << "Succesfully loaded " << tools_with_timeout.size() << " case(s)!\n";
}
...
void Board::save(const std::string &p_filePath) {
    ...
    for (int i = 0; i < m_grid.size(); ++i) {
        for (int j = 0; j < m_grid[i].size(); ++j) {
            ...
            node.append_attribute("type").set_value(static_cast<int>(m_grid[j][i]->type()));
        }
    }
